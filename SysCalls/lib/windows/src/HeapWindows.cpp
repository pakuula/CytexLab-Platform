...
void Allacator::Init() {
    ...
    this->memBlocks[0].Pointer = ::HeapAlloc(heap, 0, START_MEM_BLOCK_SIZE);
...
void Allacator::DeInit() {
    ...
    for (UINT64 i = 0; i < MAX_MEM_BLOCK; i++) {
        if (this->memBlocks[i].Pointer != NULLPTR) {
            ::HeapFree(heap, 0, this->memBlocks[i].Pointer);
        } else {
            continue;
        }
    }
    ...
    if (targetBlockID == -1) {
        for (INT64 i = 0; i < MAX_MEM_BLOCK; i++) {
            if (this->memBlocks[i].Pointer == NULLPTR) {
                ...
                this->memBlocks[i].Pointer = ::HeapAlloc(::GetProcessHeap(), 0, newBlockSize);
                if (this->memBlocks[i].Pointer == NULLPTR) {
